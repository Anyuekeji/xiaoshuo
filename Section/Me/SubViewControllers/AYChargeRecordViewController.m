//
//  AYChargeRecordViewController.m
//  AYNovel
//
...
#import "AYChargeRecordViewController.h"
#import "AYUserChargeModel.h"
#import "AYMeTableViewCell.h"
#import "AYCoinSectionModel.h"

#define  CHARGE_RECORD_HEADVIEW_HEIGHT 56


@interface AYChargeRecordViewController ()
@property(nonatomic,strong)NSMutableArray<AYUserChargeModel*> *userChargeArray;
...
@implementation AYChargeRecordViewController
...
{
    self.page =1;
    _userChargeArray= [NSMutableArray new];
...
-(void)dealWithData
{
    ...
    [_userChargeArray enumerateObjectsUsingBlock:^(AYUserChargeModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [result enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        {
            if ([obj isKindOfClass:AYUserChargeModel.class]) {
                AYUserChargeModel *chargeModel = obj;
                sumChargeNum+=[chargeModel.chargeCoinNum integerValue];
            }
        }];
...
-(UIView*)createHeadViewWithSection:(NSInteger)section
{
    ...
    CGFloat headViewHeight = CHARGE_RECORD_HEADVIEW_HEIGHT;
...
-(void)loadUserChargeRecordData:(BOOL)refresh complete:(void(^)(void)) completeBlock
{
    ...
    [ZWNetwork post:@"HTTP_Post_Charge_Record" parameters:nil success:^(id record)
     {
         if (!completeBlock) {
             [self hideHUD];
         }
         ...
         {
             NSArray *itemArray = [AYUserChargeModel itemsWithArray:record];
             ...
             if (itemArray.count>0) {
                 ...
                 if(refresh)
                 {
                     [self.userChargeArray removeAllObjects];
                     [self.sectionChargeArray removeAllObjects];
                 }
                 [self.userChargeArray safe_addObjectsFromArray:itemArray];
                 [self dealWithData];
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section
{
    ...
    return CHARGE_RECORD_HEADVIEW_HEIGHT;
}
