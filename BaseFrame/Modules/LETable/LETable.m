...
@synthesize forceRefreshEvent = _forceRefreshEvent;

#pragma mark - Initializations
+ (id) tableView {
    return [self tableViewWithTopRefreshControl:nil bottomRefreshControl:nil];
}

+ (id) tableViewWithTopRefreshControl : (LERefreshControl *) topRefreshControl {
    return [self tableViewWithTopRefreshControl:topRefreshControl bottomRefreshControl:nil];
}

+ (id) tableViewWithBottomRefreshControl : (LERefreshControl *) bottomRefreshControl {
    return [self tableViewWithTopRefreshControl:nil bottomRefreshControl:bottomRefreshControl];
}
...
+ (id) tableViewWithTopRefreshControl:(LERefreshControl *)topRefreshControl bottomRefreshControl : (LERefreshControl *) bottomRefreshControl {
    return [[self alloc] initWithTopRefreshControl:topRefreshControl bottomRefreshControl:bottomRefreshControl tableViewStyle:UITableViewStylePlain];
}
...
+ (id) tableViewWithTopRefreshControl:(LERefreshControl *)topRefreshControl bottomRefreshControl:(LERefreshControl *)bottomRefreshControl tableViewStyle : (UITableViewStyle) style {
    return [[self alloc] initWithTopRefreshControl:topRefreshControl bottomRefreshControl:bottomRefreshControl tableViewStyle:style];
}
...
+ (id) tableViewWithFrame:(CGRect)frame topRefreshControl:(LERefreshControl *)topRefreshControl bottomRefreshControl:(LERefreshControl *)bottomRefreshControl {
    return [[self alloc] initWithFrame:frame topRefreshControl:topRefreshControl bottomRefreshControl:bottomRefreshControl tableViewStyle:UITableViewStylePlain];
}
...
+ (id) tableViewWithFrame:(CGRect)frame topRefreshControl:(LERefreshControl *)topRefreshControl bottomRefreshControl:(LERefreshControl *)bottomRefreshControl tableViewStyle : (UITableViewStyle) style {
    return [[self alloc] initWithFrame:frame topRefreshControl:topRefreshControl bottomRefreshControl:bottomRefreshControl tableViewStyle:style];
}

- (instancetype) initWithTopRefreshControl : (LERefreshControl *) topRefreshControl bottomRefreshControl : (LERefreshControl *) bottomRefreshControl tableViewStyle : (UITableViewStyle) style {
...
- (instancetype) initWithFrame:(CGRect)frame topRefreshControl : (LERefreshControl *) topRefreshControl bottomRefreshControl : (LERefreshControl *) bottomRefreshControl tableViewStyle : (UITableViewStyle) style {
...
- (void) tableViewDidScroll : (UIScrollView *) scrollView {
    ...
    } else if ( _supportTopRefresh && _topRefreshControl.isAbsorb && _topRefreshControl.state == LERefreshControlStateRespond ) {   //吸附顶部回弹
        [_topRefreshControl reTransilationTo:fabs(scrollView.contentOffset.y + self.contentInset.top)];
    }
    //底部刷新动作
...
- (void) tableViewDidEndDragging : (UIScrollView *) scrollView {
    if ( _supportTopRefresh && [_topRefreshControl refreshControlStateChangedTo:LERefreshControlStateRespond] && !_inRefreshEvent) {
        [self leTableBeginRefreshAction];
    }
    
    if ( _supportBottomRefresh && [_bottomRefreshControl refreshControlStateChangedTo:LERefreshControlStateRespond] && !_inRefreshEvent ) {
        [self leTableBeginLoadMoreAction];
    }
}

- (void) leTableWillBeginRefreshOrLoadMoreAction : (BOOL) isRefreshOrNotLoadMore {
    if ( isRefreshOrNotLoadMore && _lazyLoadDelegate && [_lazyLoadDelegate respondsToSelector:@selector(leTableWillBeginRefreshAction)] ) {
        [_lazyLoadDelegate leTableWillBeginRefreshAction];
    } else if ( !isRefreshOrNotLoadMore && _lazyLoadDelegate && [_lazyLoadDelegate respondsToSelector:@selector(leTableWillBeginLoadMoreAction)] ) {
        [_lazyLoadDelegate leTableWillBeginLoadMoreAction];
    }
}

- (void) leTableBeginLoadMoreAction {
    _inRefreshEvent = YES;
    [UIView animateWithDuration:0.2f animations:^{
        ...
        [self leTableWillBeginRefreshOrLoadMoreAction:NO];
    }];
    __weak __typeof(&*self) weakSelf = self;
    if ( _lazyLoadDelegate && [_lazyLoadDelegate respondsToSelector:@selector(leTableLoadMoreChokeAction:)] ) {
        [_lazyLoadDelegate leTableLoadMoreChokeAction:^{
            [weakSelf leTableEndLoadMoreBlockAction];
        }];
    } else if ( _lazyLoadDelegate && [_lazyLoadDelegate respondsToSelector:@selector(leTableLoadMoreAction)] ) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [weakSelf.lazyLoadDelegate leTableLoadMoreAction];
...
- (void) leTableEndLoadMoreBlockAction {
    if ( _lazyLoadDelegate && [_lazyLoadDelegate respondsToSelector:@selector(leTableLoadMoreActionDone)] ) {
        [_lazyLoadDelegate leTableLoadMoreActionDone];
    }
    [self reloadData];
    [UIView animateWithDuration:0.2f animations:^{
...
- (void) leTableBeginRefreshAction {
    _inRefreshEvent = YES;
    [UIView animateWithDuration:0.2f animations:^{
        ...
        [self leTableWillBeginRefreshOrLoadMoreAction:YES];
    }];
    __weak __typeof(&*self) weakSelf = self;
    if ( _lazyLoadDelegate && [_lazyLoadDelegate respondsToSelector:@selector(leTableRefreshChokeAction:)] ) {
        [_lazyLoadDelegate leTableRefreshChokeAction:^{
            [weakSelf leTableEndRefreshBlockAction];
        }];
    } else if ( _lazyLoadDelegate && [_lazyLoadDelegate respondsToSelector:@selector(leTableRefreshAction)] ) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [weakSelf.lazyLoadDelegate leTableRefreshAction];
            dispatch_async(dispatch_get_main_queue(), ^{
                [weakSelf leTableEndRefreshBlockAction];
            });
        });
...
- (void) leTableEndRefreshBlockAction {
    if ( _lazyLoadDelegate && [_lazyLoadDelegate respondsToSelector:@selector(leTableRefreshActionDone)] ) {
        [_lazyLoadDelegate leTableRefreshActionDone];
    }
    [self reloadData];
...
- (void) checkIfLoadNotAnyMore {
    if ( _supportBottomRefresh ) {
        ...
        if ( _lazyLoadDelegate && [_lazyLoadDelegate respondsToSelector:@selector(leTableLoadNotAnyMore)] ) {
            if ( [_lazyLoadDelegate leTableLoadNotAnyMore] ) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ( self->_supportTopRefresh && !self->_hadRefreshing ) {
            ...
            [UIView animateWithDuration:0.25 animations:^{
                self.contentOffset = CGPointMake(0, [self.topRefreshControl awakenHeight] - 1.0f - self.contentInset.top);
            } completion:^(BOOL bl){
