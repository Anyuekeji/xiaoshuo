...
#error LEHud is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "LEHud.h"
#import <QuartzCore/QuartzCore.h>

NSString * const LEHudDidReceiveTouchEventNotification = @"LEHudDidReceiveTouchEventNotification";
NSString * const LEHudWillDisappearNotification = @"LEHudWillDisappearNotification";
NSString * const LEHudDidDisappearNotification = @"LEHudDidDisappearNotification";
NSString * const LEHudWillAppearNotification = @"LEHudWillAppearNotification";
NSString * const LEHudDidAppearNotification = @"LEHudDidAppearNotification";

NSString * const LEHudStatusUserInfoKey = @"LEHudStatusUserInfoKey";

#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 70000
static const CGFloat LEHudRingRadius = 14;
static const CGFloat LEHudRingThickness = 1;
static const CGFloat LEHudParallaxDepthPoints = 10;
#else
static const CGFloat LEHudRingRadius = 14;
static const CGFloat LEHudRingThickness = 6;
#endif

@interface LEHud ()

@property (nonatomic, readwrite) LEHudMaskType maskType;
...
- (void)showProgress:(float)progress
            ...
            maskType:(LEHudMaskType)hudMaskType;
...
@synthesize hudWarningImage = _uihudWarningImage;
#endif

+ (LEHud*)sharedView {
    static dispatch_once_t once;
    static LEHud *sharedView;
...
+ (void)show {
    [[self sharedView] showProgress:-1 status:nil maskType:LEHudMaskTypeNone];
}

+ (void)showWithStatus:(NSString *)status {
    [[self sharedView] showProgress:-1 status:status maskType:LEHudMaskTypeNone];
}
...
+ (void)showProgress:(float)progress {
    [[self sharedView] showProgress:progress status:nil maskType:LEHudMaskTypeNone];
}

+ (void)showProgress:(float)progress status:(NSString *)status {
    [[self sharedView] showProgress:progress status:status maskType:LEHudMaskTypeNone];
}
...
+ (void)showWarningWithStatus:(NSString*)string {
    if ([string isKindOfClass:[NSString class]]) {
        [self showImage:[[self sharedView] hudWarningImage] status:string];
    }
}

+ (void)showImage:(UIImage *)image status:(NSString *)string {
    NSTimeInterval displayInterval = [[LEHud sharedView] displayDurationForString:string];
...
- (void)drawRect:(CGRect)rect {
    ...
    switch (self.maskType) {
        case LEHudMaskTypeBlack: {
            [[UIColor colorWithWhite:0 alpha:0.5] set];
            CGContextFillRect(context, self.bounds);
            break;
        }
        case LEHudMaskTypeGradient: {
...
- (NSDictionary *)notificationUserInfo {
    return (self.stringLabel.text ? @{LEHudStatusUserInfoKey : self.stringLabel.text} : nil);
}
...
- (void)overlayViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent *)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:LEHudDidReceiveTouchEventNotification object:event];
}

#pragma mark - Master show/dismiss methods
- (void)showProgress:(float)progress status:(NSString*)string maskType:(LEHudMaskType)hudMaskType {
    ...
    if(self.maskType != LEHudMaskTypeNone) {
    ...
    if(self.alpha != 1) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:LEHudWillAppearNotification
                                                            object:nil
                                                          userInfo:userInfo];
        ...
        [UIView animateWithDuration:0.15
                         ...
                         completion:^(BOOL finished){
                             [[NSNotificationCenter defaultCenter] postNotificationName:LEHudDidAppearNotification
                                                                                 object:nil
                                                                               userInfo:userInfo];
...
- (void)dismiss {
    NSDictionary *userInfo = [self notificationUserInfo];
    [[NSNotificationCenter defaultCenter] postNotificationName:LEHudWillDisappearNotification
                                                        object:nil
                                                      userInfo:userInfo];
    ...
    [UIView animateWithDuration:0.15
                     ...
                     animations:^{
                         ...
                         if(self.alpha == 0 || self.hudView.alpha == 0) {
                             ...
                             [[NSNotificationCenter defaultCenter] postNotificationName:LEHudDidDisappearNotification
                                                                                 object:nil
                                                                               userInfo:userInfo];
...
- (CAShapeLayer *)ringLayer {
    if(!_ringLayer) {
        ...
        _ringLayer = [self createRingLayerWithCenter:center radius:LEHudRingRadius lineWidth:LEHudRingThickness color:self.hudRingForegroundColor];
...
- (CAShapeLayer *)backgroundRingLayer {
    if(!_backgroundRingLayer) {
        CGPoint center = CGPointMake(CGRectGetWidth(hudView.frame)/2, CGRectGetHeight(hudView.frame)/2);
        _backgroundRingLayer = [self createRingLayerWithCenter:center radius:LEHudRingRadius lineWidth:LEHudRingThickness color:self.hudRingBackgroundColor];
...
#pragma mark - Getters
- (NSTimeInterval)displayDurationForString:(NSString*)string {
    return MIN((float)string.length*0.07 + 0.3, 5.0);
}

- (BOOL)isClear { // used for iOS 7
    return (self.maskType == LEHudMaskTypeClear || self.maskType == LEHudMaskTypeNone);
}
...
- (UIView *)hudView {
    if(!hudView) {
...
#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 70000
        {
            ...
            effectX.minimumRelativeValue = @(-LEHudParallaxDepthPoints);
            effectX.maximumRelativeValue = @(LEHudParallaxDepthPoints);
            
            UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath: @"center.y" type: UIInterpolatingMotionEffectTypeTiltAlongVerticalAxis];
            effectY.minimumRelativeValue = @(-LEHudParallaxDepthPoints);
            effectY.maximumRelativeValue = @(LEHudParallaxDepthPoints);
...
- (UIImage *)hudSuccessImage {
    ...
    return [UIImage imageNamed:@"icon_promt_complete_black"];
}
...
- (UIImage *)hudErrorImage {
    ...
    return [UIImage imageNamed:@"icon_promt_wrong"];
}

- (UIImage *)hudWarningImage {
#if __IPHONE_OS_VERSION_MIN_REQUIRED >= 50000
    if(_uihudWarningImage == nil) {
        _uihudWarningImage = [[[self class] appearance] hudWarningImage];
    }
    
    if(_uihudWarningImage != nil) {
        return _uihudWarningImage;
    }
#endif
    
    return [UIImage imageNamed:@"icon_promt_warning_black"];
}
